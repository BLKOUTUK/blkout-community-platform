    ...
    try {
      ...
      const results = await chrome.scripting.executeScript({
        ...
        function: () => {
          ...
          extractedData.eventData = eventData;
  ...
  async submitToQueue() {
    ...
    try {
      ...
      const localSubmission = {
        ...
        url: contentData.metadata?.url || 'current-page',
        offline: true
      };

      this.saveToLocalStorage('submitted', localSubmission);
      this.showError('API unavailable - saved locally for sync later');
      this.updateStats('submitted');
    }
  }
